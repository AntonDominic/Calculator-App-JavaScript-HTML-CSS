...
function evaluateExpression() {
  ...
  // checks if evalResult isNaN  or infinite. If it is, returns a space character ' '
...
function negate() {
  // Negate if the expression is empty and the result is present
...
function decimal(value) {
  if (!expression.startsWith(".") && !isNaN(expression.slice(-1))) {
    addValue(value);
  }
}
